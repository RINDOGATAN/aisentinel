...
| `CROSS_LOGIN_ISSUERS` | C | Hosted | The `iss` values accepted on a signed cross-login token; unset or empty refuses every signed token | Hosted operator, in step with the sender | n/a |
