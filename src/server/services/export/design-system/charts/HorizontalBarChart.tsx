    ...
    <View style={s.container}>
      {rows.map((r, i) => {
        ...
        const fillPct = Math.max(
          r.value === 0 && !showEmptyAsZero ? 0 : ratio * 100,
          r.value > 0 ? 8 : 3,
        );
        const fillColor = r.color ?? tokens.color.brand.accent;
        const labelColor = r.labelColor ?? tokens.color.text.primary;
        return (
          <View key={i} style={s.row} wrap={false}>
            <Text style={[s.label, { color: labelColor, width: labelWidth }]}>
              {r.label}
            </Text>
              ...
              <View
                style={[
                  s.fill,
                  {
                    width: `${fillPct}%`,
                    backgroundColor:
                      r.value === 0 ? tokens.color.surface.subtleAlt : fillColor,
                  },
                ]}
              >
                {r.value > 0 ? (
                  ...
                  <Text style={[s.countText, { color: tokens.color.text.muted }]}>
                    0
                  </Text>
                )}
              </View>
