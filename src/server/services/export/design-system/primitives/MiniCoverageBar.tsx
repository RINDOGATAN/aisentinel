...
const toneForCoverage = (pct: number): SemanticTone => {
...
export function MiniCoverageBar({
  ...
  const autoTone = tone ?? toneForCoverage(pct);
  const fillColor = tokens.color.semantic[autoTone].solid;
  const display =
    format === "percent" ? `${Math.round(pct * 100)}%` : `${value}/${total}`;
  return (
    <View style={s.row} wrap={false}>
      <Text style={s.label}>{label}</Text>
      <View style={s.track}>
        <View
          style={[s.fill, { width: `${pct * 100}%`, backgroundColor: fillColor }]}
        />
      </View>
