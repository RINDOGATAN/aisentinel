 ...
 * (byte-compatible with Dealroom, DPO Central, and the TODO.LAW storefront),
 * the activateOffline flow (email binding, expiry, activation limits,
 ...
 * Hermetic: real router + middleware chain over a module-mocked Prisma.
 */
...
import { skillsRouter } from "@/server/routers/governance/skills";
...
/** Real router caller with the real middleware chain, for a given org role. */
function callerWithRole(role: OrganizationRole, email = BUYER_EMAIL) {
  ...
  return skillsRouter.createCaller(
    createInnerTRPCContext({
      session: sessionFor("user-1", email),
      getCookie: () => undefined,
    })
  );
}
...
beforeAll(() => {
  ...
  process.env.AISENTINEL_INSTANCE_ID = "inst_test";
});
...
describe("skills.activateOffline role gate", () => {
  ...
  it("denies a MEMBER and an AI_OFFICER", async () => {
    for (const role of ["MEMBER", "AI_OFFICER"] as const) {
      await expect(
...
describe("skills.activateOffline flow", () => {
  it("activates a valid licence: creates customer + org link, entitlement, activation", async () => {
    ...
    // Audit trail (New Module Checklist: audit entry on every mutation)
